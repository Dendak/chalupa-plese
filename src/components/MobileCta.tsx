...
export function MobileCta() {
  ...
  return (
    <div
      className={clsx(
        'fixed inset-x-3 bottom-3 z-40 flex gap-2 transition-all duration-500 sm:hidden',
        show ? 'translate-y-0 opacity-100' : 'pointer-events-none translate-y-6 opacity-0',
      )}
      style={{ paddingBottom: 'env(safe-area-inset-bottom)', transitionTimingFunction: 'cubic-bezier(0.16,1,0.3,1)' }}
    >
      <a href={`tel:${OWNER.phone.replace(/\s/g, '')}`} aria-label="Zavolat" className="glass grid size-12 shrink-0 place-items-center rounded-full text-ink shadow-lifted">
        <Phone className="size-5" />
      </a>
      <a href="#obsazenost" className="btn-accent flex-1 py-3.5 shadow-lifted">
        <CalendarDays className="size-4" /> Volné termíny a poptávka
      </a>
    </div>
  );
}
