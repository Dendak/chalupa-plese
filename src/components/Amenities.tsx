    ...
    <section id="vybaveni" className="bg-forest text-white dark:bg-surface dark:text-ink">
      <div className="container-x py-24 sm:py-32">
        <div className="[&_.eyebrow]:text-gold [&_p]:text-white/75 dark:[&_p]:text-ink-muted">
          <SectionHeading eyebrow={t.amenities.eyebrow} title={t.amenities.title} text={t.amenities.text} />
        </div>
        <div className="mt-14 grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {AMENITIES.map((g, gi) => (
            <Reveal key={g.group} delay={gi * 0.08}>
              <h3 className="font-display text-2xl font-medium text-gold">{t.amenities.groups[g.group]}</h3>
              <ul className="mt-5 space-y-3">
                {g.items.map((it) => (
                  <li key={it.key} className="flex items-start gap-3 text-sm leading-snug text-white/90 dark:text-ink">
                    ...
                    <span className="pt-1.5">{t.amenities.items[it.key]}</span>
                  </li>
                ))}
              </ul>
            </Reveal>
